...
import { CheckSquare, Square, Lock, User as UserIcon, ShieldCheck } from 'lucide-react';
...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleRecoverySubmit = async (e: React.FormEvent) => {
    ...
    if (settings.emailjs.serviceId && settings.emailjs.templateIdRecovery && settings.emailjs.publicKey) {
      try {
        await emailjs.send(
          settings.emailjs.serviceId,
          settings.emailjs.templateIdRecovery,
          { 
            to_email: recoveryEmail,
            message: 'Solicitação de recuperação de senha recebida.', 
            link: window.location.origin + '/reset-password-mock' // Mock link
          },
          settings.emailjs.publicKey
        );
        alert(`Instruções de recuperação enviadas para: ${recoveryEmail} (Via EmailJS)`);
      } catch (error) {
        console.error('EmailJS Error:', error);
        alert('Erro ao enviar e-mail via serviço externo. Verifique as configurações ou tente novamente mais tarde.');
      }
    } else {
      // Fallback simulation
      setTimeout(() => {
        alert(`(Simulação) Instruções de recuperação enviadas para: ${recoveryEmail}.\n\nNota: Configure o EmailJS no Painel Admin para envio real.`);
      }, 1000);
    }
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            Digite seu e-mail abaixo. Enviaremos um link seguro para redefinição de senha.
          </p>
