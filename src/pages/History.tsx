...
import { 
  ...
  ArrowRight, FileText, CheckCircle2, TrendingUp, ArrowUpDown, AlertCircle, Printer, Save, Loader2
} from 'lucide-react';
...
export const History: React.FC = () => {
  ...
  useEffect(() => {
    if (reportModalOpen && selectedPatientId) {
      const loadNotes = async () => {
        setIsLoadingNotes(true);
        try {
          // No histórico, tentamos pegar a data da avaliação mais recente para buscar o agendamento
          // Ou se o usuário clicou em um detalhe específico antes, poderíamos usar aquela data.
          // Como o relatório é geral, pegamos a data da última avaliação registrada no histórico.
          let targetDate = undefined;
          if (history.length > 0) {
             // Ordena por data decrescente para pegar a última
             const sorted = [...history].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
             targetDate = sorted[0].date;
          }

          const appointment = await agendaService.getAppointmentForReport(selectedPatientId, targetDate);
