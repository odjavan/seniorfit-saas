  ...
  const handleSimulateWebhook = () => {
    ...
    try {
      ...
      if (result.action === 'created') {
        addToast(`Assinante criado: ${result.user.name}`, 'success');
      } else {
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <Button onClick={handleSave} variant="blue" isLoading={isSaving}>
          <Save size={18} className="mr-2" /> {isSaving ? 'SALVANDO...' : 'SALVAR TUDO'}
        </Button>
      </div>
          ...
          {activeTab === 'eduzz' && (
            <div className="space-y-8 animate-fade-in">
              ...
              <div className="space-y-4">
                <Input label="URL do Aplicativo" value={settings.eduzz.appUrl} onChange={(e) => handleChange('eduzz', 'appUrl', e.target.value)} />
          ...
          {activeTab === 'emailjs' && (
            <div className="space-y-8 animate-fade-in">
              ...
              <div className="bg-purple-50 p-6 rounded-xl border border-purple-100 mb-6">
                <h3 className="text-lg font-bold text-purple-900 mb-3 flex items-center"><Bell className="mr-2" size={20} /> Diretrizes de Notificação Push</h3>
                <ul className="list-disc list-inside space-y-2 text-sm text-purple-800">
                  <li><strong>Inatividade (3 dias):</strong> Alerta motivacional.</li>
                  <li><strong>Inatividade (7 dias):</strong> Risco de destreino.</li>
                  <li><strong>Aniversário:</strong> Mensagem automática.</li>
                  <li><strong>Renovação:</strong> Aviso 7 dias antes.</li>
                </ul>
              </div>
